...
>
  ...
  <div class="relative rounded-xl bg-[#06080a] p-5 sm:p-6">
    ...
    ><code class="block">{#each script as line, i (i)}{#if line.kind === 'spacer'}<span class="block h-2"></span>{:else if busyIdx === i}<span class="block"><span class="inline-flex items-center gap-2 text-zinc-500"><span class="term-spinner" aria-hidden="true"></span><span>{line.kind === 'output-muted' && line.text.startsWith('uploading') ? 'uploading' : line.kind === 'output-muted' && line.text.startsWith('restoring') ? 'restoring' : 'working'}<span class="term-dots"></span></span></span></span>{:else}<span class="block">{#if line.kind === 'prompt'}<span class="text-emerald-500/80">$</span> <span class="text-zinc-100">{line.text.slice(0, visible[i])}</span>{:else if line.kind === 'output-emerald'}<span class="text-emerald-300">{line.text.slice(0, visible[i])}</span>{:else if line.kind === 'output-muted'}<span class="text-zinc-500">{line.text.slice(0, visible[i])}</span>{:else}<span class="text-zinc-300">{line.text.slice(0, visible[i])}</span>{/if}{#if activeIdx === i && visible[i] < line.text.length && !line.instant}<span class="term-caret animate-blink bg-emerald-400"></span>{/if}{#if visible[i] === 0}{'\u00A0'}{/if}</span>{/if}{/each}</code></pre>
  </div>
</div>
