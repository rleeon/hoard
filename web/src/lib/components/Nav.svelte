...
>
  ...
  >
    <a
      href={$localeHref('/')}
      class="nav-pill glow pop-self group flex items-center ring-focus md:justify-self-start"
    ...
    <div class="hidden items-center gap-1 md:flex md:justify-self-center">
      <a
        ...
        class="nav-pill glow pop-self ring-focus text-sm {isActive('/download')
          ? 'nav-pill-active'
      ...
      <a
        ...
        class="nav-pill glow pop-self ring-focus text-sm {isActive('/pricing')
          ? 'nav-pill-active'
          : 'text-ink-soft hover:text-ink'}"
      >
        {$_('nav.pricing')}
      ...
      <a
        href={$localeHref('/cli')}
        class="nav-pill glow pop-self ring-focus text-sm {isActive('/cli')
          ? 'nav-pill-active'
          : 'text-ink-soft hover:text-ink'}"
      >
        {$_('nav.cli')}
      ...
      <a
        ...
        class="nav-pill glow pop-self ring-focus text-sm {isActive('/guides')
          ? 'nav-pill-active'
          : 'text-ink-soft hover:text-ink'}"
      >
        {$_('nav.guides')}
      ...
      <a
        ...
        class="nav-pill glow pop-self ring-focus text-sm {isActive('/help')
          ? 'nav-pill-active'
          : 'text-ink-soft hover:text-ink'}"
      >
        {$_('nav.help')}
    ...
    <div class="hidden items-center gap-3 md:flex md:justify-self-end">
      ...
      >
        <summary
          class="nav-pill glow pop-self flex cursor-pointer list-none items-center gap-1.5 text-sm text-ink-soft ring-focus [&::-webkit-details-marker]:hidden"
          aria-label="Language"
        >
          <!-- Same spin as the phone's globe: one language of motion for the
               same control on both sizes. -->
          <!-- The pop is off while the panel is open: the animation drives
               `transform`, so it would drop the globe's 180deg turn for its
               360ms. -->
          <Globe
            data-anim={langOpen ? undefined : 'pop'}
        ...
        >
          {#each LOCALES as l (l)}
            <a
              ...
              class="glow flex items-center justify-between gap-3 px-3 py-2 text-sm transition-colors hover:bg-ink/5 {l === active
                ? 'text-ink'
                : 'text-ink-soft hover:text-ink'}"
            >
              {LOCALE_NAMES[l]}
      ...
      {:else if $session}
        <a
          href="/account"
          class="glow pop-self group flex items-center gap-2 rounded-full border border-line bg-surface py-1 pl-1 pr-3 ring-focus transition-colors hover:border-line-strong"
        >
      ...
      {:else}
        <a
          href="/login"
          class="glow pop-self inline-flex h-9 items-center rounded-lg bg-accent px-4 text-sm font-medium text-pine ring-focus transition-colors hover:bg-emerald-300"
        >
          {$_('nav.signin')}
        </a>
      {/if}
      ...
      <a
        ...
        aria-label="Discord"
        title="Discord"
        class="glow pop-self grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#5865F2] text-white ring-focus transition-colors hover:bg-[#4752C4]"
      >
        <DiscordIcon data-anim="pop" class="h-[18px] w-[18px]" />
      </a>
    </div>
    ...
    <div class="flex items-center gap-1 md:hidden">
      ...
      <button
        class="glow anim-host pop-self grid h-10 w-auto min-w-10 place-items-center gap-1 rounded-lg px-2 text-ink-soft ring-focus hover:bg-ink/5"
        onclick={() => {
          langInstant = false;
      ...
      >
        <span class="flex items-center gap-1.5">
          ...
          <Globe
            data-anim={langMobile ? undefined : 'pop'}
      ...
      <button
        class="glow pop-self grid h-10 w-10 place-items-center rounded-lg text-ink-soft ring-focus hover:bg-ink/5"
        onclick={() => {
          open = !open;
          langMobile = false;
        }}
  ...
  {#if open}
    ...
    >
      <div class="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-3">
        <a class="glow rounded-md px-3 py-2.5 text-ink hover:bg-ink/5" href={$localeHref('/download')} onclick={() => (open = false)}>
          {$_('nav.download')}
        </a>
        <a class="glow rounded-md px-3 py-2.5 text-ink hover:bg-ink/5" href={$localeHref('/pricing')} onclick={() => (open = false)}>
          {$_('nav.pricing')}
        </a>
        <a class="glow rounded-md px-3 py-2.5 text-ink hover:bg-ink/5" href={$localeHref('/cli')} onclick={() => (open = false)}>
          {$_('nav.cli')}
        </a>
        <a class="glow rounded-md px-3 py-2.5 text-ink hover:bg-ink/5" href={$localeHref('/guides')} onclick={() => (open = false)}>
          {$_('nav.guides')}
        </a>
        <a class="glow rounded-md px-3 py-2.5 text-ink hover:bg-ink/5" href={$localeHref('/help')} onclick={() => (open = false)}>
          {$_('nav.help')}
        </a>
        {#if $session}
          <a class="glow rounded-md px-3 py-2.5 text-ink hover:bg-ink/5" href="/account" onclick={() => (open = false)}>
            {$_('nav.account')}
          </a>
          <button class="glow rounded-md px-3 py-2.5 text-left text-ink hover:bg-ink/5" onclick={signOut}>
            {$_('nav.signout')}
          </button>
        {:else}
          <a
            class="glow mt-1 inline-flex items-center justify-center rounded-md bg-accent px-3 py-2.5 font-medium text-pine hover:bg-emerald-300"
            href="/login"
            onclick={() => (open = false)}
          >
            {$_('nav.signin')}
        ...
        {/if}
        <a
          class="glow anim-host mt-1 inline-flex items-center justify-center gap-2 rounded-md bg-[#5865F2] px-3 py-2.5 font-medium text-white hover:bg-[#4752C4]"
          href={DISCORD_URL}
          target="_blank"
          rel="noopener noreferrer"
          onclick={() => (open = false)}
  ...
  {#if langMobile}
    ...
    >
      <div class="grid grid-cols-2 gap-1 px-4 py-3" data-sveltekit-preload-data="tap">
        {#each LOCALES as l (l)}
          <a
            class="glow flex items-center justify-between rounded-md px-3 py-2.5 text-sm hover:bg-ink/5 {l === active
              ? 'text-ink'
              : 'text-ink-soft'}"
            href={langTarget(l)}
            hreflang={l}
